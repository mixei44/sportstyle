...
const csrfField = document.querySelector('#changeForm > input[name="csrfmiddlewaretoken"]');
...
const oldPasswordState = {field: document.getElementById('old_password'), errors: {}, isValid: false, validValue: undefined};
const stateDictionary = {new_password1: newPassword1State, new_password2: newPassword2State, old_password: oldPasswordState};

document.getElementById('changeForm').addEventListener('submit', async evt => {
    evt.preventDefault();
    ...
    try {
        ...
        document.getElementById('changeAlert').classList.remove('d-none');
        disableFields(stateDictionary, ['changeSubmit']);
    };
});
...
oldPasswordState.field.addEventListener('focusout', async evt => {
    const oldPasswordValue = evt.target.value;
    if (!validatePassword1(oldPasswordValue, oldPasswordState)) {
        return;
    };
    if (oldPasswordState.isValid) {
        return;
    };
    sendRequest(oldPasswordState, csrfField.value, {'password1': oldPasswordValue, 'names': {password1: 'current password'}}, (responseData) => {
        if (responseData['password1'] && responseData['code']) {
            oldPasswordState.errors[responseData['code']] = responseData['password1'];
            invalidateField(oldPasswordState.field, responseData['password1']);
        } else {
            makeFieldValid(oldPasswordState.field);
            oldPasswordState.isValid = true;
            oldPasswordState.validValue = oldPasswordValue;
        };
    });
});
